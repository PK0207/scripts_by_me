# -*- coding: utf-8 -*-
"""
Created on Fri Apr  4 12:52:49 2025
...
import arviz as az #convart pandas dataframe to a data type compatible with corner (corner+pandas is now deprecated)
import numpy as np
import pandas as pd
...
def grid_from_radius(targ_d, inc):
    ...
    for r_idx, r in enumerate(r_grid):
        for azi_idx, azi in enumerate(tan_phi):
            ...
            xgrid[r_idx, azi_idx] = x_dummy / np.cos(inc * (np.pi / 180.0))
...
def vrad_to_vobs(r_grid, phi_grid, inc):
    ...
    vobs = [v_k * np.sin(inc * (np.pi / 180.0)) * np.sin(phi * (np.pi / 180.0)) for phi in phi_grid]
...
def fit_H2Model(vradD,fluxD,errrD,ig,pb):
    ...
    H2P=H2M.make_params(LyA01=dict(value=ig[0],vary=False),
                        ...
                        LyA14=dict(value=ig[2],min=pb[2][0],max=pb[2][1]),
                        LyA17=dict(value=ig[3],min=pb[3][0],max=pb[3][1]),
...
initGuess=[-10.75,       -10.85,       -13.25,        -13.5,       5.0,      1.5,       2200.0,        0.45,      0.5,       -10.0] #initial parameter guesses
paramBnds=[[-11.0,-10.5],[-11.0,-10.7],[-14.0,-10.0],[-14.0,-10.0],[2.0,7.0],[0.0,1.99],[500.0,5000.0],[-2.5,2.5],[0.0,50.0],[-16.0,-9.0]] #parameter bounds
#to start, just set all parameters to the middle of each parameter range (i.e. the center of the 10D space)
#^that was a disaster, instead using (-10.75,-10.85,-11.0,-11.0,6.0,1.75,2200.0,0.5,0.03,-9.82)
#based on Nicole's model, extended the lower LyA bounds to 1e-14 and set the initial guesses to 1e-13
...
plt.title(f"PDS 70: Best Fit ($\\chi^2_R$ = {bestChi[1]:.3f})",fontsize=20)
...
nWalkers=20 #how many walkers will roam the parameter space
nSteps=200 #how many samples are drawn by each walker, how many "steps" they take
...
#100/5000/500/25 predicted to take ~103 hours
#100/1000/100/25 predicted to take ~18 hours
#20/500/50/25 predicted to take ~2 hours
#20/200/20/10 predicted to take ~30 minutes, actually took ~57 minutes (200 more steps took ~13 minutes) (1000 more took ~75 minutes)
...
paramVar=paramName[2:] #remove the fixed parameters
...
if newChain:
    ...
    emcee_corner=corner.corner(az.convert_to_inference_data(result_emcee.chain),
                               ...
                               truths=list(result_emcee.params.valuesdict().values())[2:]) #remove the first two fixed params
    
    ppow,pscl=plotPower(all_dataflux)
    fig,ax=plt.subplots(2,1,sharex=True)
    plt.suptitle('PDS 70 MCMC Results',fontsize=24)
    ...
    if hasattr(result_emcee, "acor"):
        ...
        print("\nUnable to determine autocorrelation time for the parameters")
        for i in range(len(paramVar)):
    ...
    if save:
        ...
        file_mcmc=open('PDS70_MCMC_Results_and_Params.obj','wb')
...
if contMCMC:
    ...
    runFiles=glob('PDS70_MCMC*.obj') #PDS 70 Full MCMC Results and Params Run3.obj
