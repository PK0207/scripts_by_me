# -*- coding: utf-8 -*-
"""
Created on Wed Apr 23 10:15:19 2025
...
#!!!using V4046 for now, replace with values from my reconstruction at some point
...
prior_dict = {'z': [2.0, 7.0],
              ...
              'q': [-2.5, 2.5],
              'rchar': [0.0, 50.0],
              'MH2': [1.0e-5, 1.0e-1], #Nicole said to use these bounds, lower mass bounds were experimental
...
#5**6 --> 75 min
#10**6 --> 80 hrs
res=5 #resolution for all parameters
...
MH2_vals = np.logspace(-5,-1,res) #logspace will space things ount nicely within the range of input powers of 10
...
#for param_list in tqdm(list(product(z_vals, gamma_vals, T_vals, q_vals, rchar_vals, MH2_vals, LyA14flux, LyA17flux))):
    ...
    outF = open(f"EP_Cha_modeling_chi2_H2_{timeFmt}.txt",'a') #save model results to output file
    outF.write(str_to_write+'\n')
    outF.close()
    
#Read in file with chi^2 values saved, to pull out the best-fit model
data_file = f"EP_Cha_modeling_chi2_H2_{timeFmt}.txt"
...
fig,ax=plt.subplots()
plt.title("EP Cha: Best-Fit H2 Model Emission Lines")
