...
aperwid = [h['APERWID'] for h in headers]
exptime = [h['EXPTIME'] for h in headers]
plt.figure(dpi=200)
plt.scatter(times, fringe_rms, s=5, c=aperwid, cmap = 'tab20')
plt.title('Fringe STD for each image')
plt.xlabel('Image Number')
plt.ylabel('Standard Deviation of the sub-region')
cax = plt.colorbar(ticks = [1.2, 1.6, 2.0, 6.0])
cax.set_label('Aperture Width', rotation=270)
plt.show()
#%% Plot power spectrum
...
fringe_power = []
for image in images:
    mean_linecut = normalize(image[165, 1100:1300])
    N = 1*200
    yf = fft(mean_linecut)
    xf = fftfreq(N)
...
plt.figure(dpi=200)
plot = plt.scatter(times, fringe_power, s=5, c=exptime, cmap = 'viridis', label=f'fringe frequency = {fringe_freq[0]}')
plt.title('Power of fringing from fft for each image')
plt.xlabel('Image Number')
plt.ylabel('Power of highest peak in fft')
cax = plt.colorbar()#plot, ticks = [1.2, 1.6, 2.0, 6.0])
cax.set_label('Exposure time', rotation=270, fontsize=10, )
plt.legend()
plt.show()
...
div_im_std = np.std(div_im[165, 1100:1300])
def std_fringe(im):
...
for i in range(len(images[:1])):
    ...
    for shift in shift_dict:
        ...
        #Plot fringe section
        plt.imshow(div_im[140:190, 1100:1300], origin='lower')
        plt.colorbar()
        plt.title(f'{data_type}, {i} fringe region')
        plt.show()
        #Plot linecut
        plt.plot(np.arange(300), div_im[165, 1100:1300])
        plt.title(f'{data_type}, {i} fringe region linecut')
        plt.xlabel('x pixel')
...
sig = normalize(images[1][165, 1100:1300])
win = normalize(images[0][165, 1100:1300])
